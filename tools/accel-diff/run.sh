...
#
#   tools/accel-diff/run.sh [haystack_bytes]
set -euo pipefail
cd "$(dirname "$0")/../.."

BYTES="${1:-262144}"
HAY="testdata/bench_haystack.txt"
OUT="$(mktemp -d)/accel_diff"
...
echo "building..."
roc build --output="$OUT" tools/accel-diff/accel_diff.roc >/dev/null 2>&1

# `a b` so the runner cannot be constant-folded whole; the haystack path is last
RESULT="$("$OUT" a b "$HAY")"
echo "$RESULT"
echo "$RESULT" | tail -1 | grep -Eq '^([0-9]+)/\1 accel == plain$'
