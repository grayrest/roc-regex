...
The default regex engine of this repository, the `Regex` module. The
Rust-`regex` port it grew up beside lives in `package-dfa/` (`Dfa`) and is kept
as an oracle.

`Regex` is a port of [RE#](https://github.com/ieviev/resharp)
...
Like `Dfa`, `Regex.compile("…")` on a literal pattern is evaluated during
...
rx : Regex.Pattern
rx = Regex.build("_*cat_*&_*dog_*")   # a haystack containing both words

Regex.find_all(rx, bytes)          # List({ start, end }) byte offsets, non-overlapping, leftmost-longest
Regex.find_all_str(rx, "…")        # the same on a Str
Regex.is_match / count / find / first_end / longest_end / replace_all / split
```
...
- `^` and `$` are line anchors always (RE# is always multiline); `\A` and
  `\Z` are the input anchors. `\z` is accepted as well, because RE#'s and
  Rust's corpora are written in it and the differentials feed those pattern
  strings to both engines unchanged; `\A`/`\Z` is the pair this engine
  spells.
...
- Unicode classes are `Uni`'s tables (Rust-derived), shared with `Dfa`.
...
| module | what |
|---|---|
| `Regex` | the API, `build`/`compile` (parse → minterms → arena → rewrites → automaton → accelerators), introspection helpers |
...
tools/bench/run.sh                                     # sharp_* rows next to Dfa and Rust
...
- Performance on 256 KB of prose is within 1–2.5x of Rust's meta engine on
  the bench patterns and ahead of `Dfa` on classes and boolean patterns;
  the literal alternation is the outlier (no literal-set accelerator in
  RE#'s design). Numbers: design log, M4 stages.

The design log and plans predate the rename and call this module `Sharp`;
`package-sharp/` became `package/` and `Sharp` became `Regex` on 2026-09-07.
